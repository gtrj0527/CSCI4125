...
/*dumb tests*/
SELECT pers.pers_id,pers.last_name,pers.first_name,pers.mi, p.comp_id
FROM works w
...
SELECT pers.pers_id, pers.last_name,pers.first_name, pers.mi
FROM works w
JOIN person pers ON pers.pers_id =w.pers_id
JOIN position p ON p.pos_code=w.pos_code
WHERE comp_id = '1113' 
...
SELECT pos.pay_rate, pers.pers_id, pers.last_name, pers.first_name, pers.mi, pos.comp_id, pos.pay_type
FROM person pers
JOIN works w ON pers.pers_id = w.pers_id
JOIN position pos ON pos.pos_code = w.pos_code
--JOIN company comp ON comp
--NATURAL JOIN position p;
WHERE comp_id = '1111' --OR comp_id = '1112' OR  comp_id = '1111'
    --AND end_date IS NULL 
    --AND pay_type = 'S'
ORDER BY pos.pay_rate DESC;
